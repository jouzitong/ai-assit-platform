...
echo "Starting db-engine service without proxy inheritance..."

echo "Preparing db-engine module dependencies..."
mvn -f app/app-platform-db-engine/pom.xml -pl boot -am \
  -DskipTests \
  install

exec mvn -f app/app-platform-db-engine/boot/pom.xml spring-boot:run \
