...
echo "Starting gateway service without proxy inheritance..."

echo "Preparing gateway module dependencies..."
mvn -f app/app-gateway/pom.xml -pl boot -am \
  -DskipTests \
  install

exec mvn -f app/app-gateway/boot/pom.xml spring-boot:run \
