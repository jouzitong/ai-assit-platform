...
description: Use when deterministic Render JSON validation returns structured errors and a bounded repair loop must apply minimal catalog-safe changes, revalidate, and emit a ValidationReport without weakening security rules.
---
...
2. Require the current document, its validation report, catalog revision, and prior attempt count.
...
2. Replace unsupported components or properties only with entries confirmed by the runtime component catalog.
