...
- 明确 `BizException`、错误码 `code`、HTTP 状态异常的边界。
...
- 需要表达 HTTP 状态语义时，才使用带 HTTP status 的异常，不要只靠错误文案区分。
...
- 只有在该场景明确要表达 REST 资源语义、并且希望返回稳定 404 含义时，才使用 `ResourceNotFindException`。
- 换句话说：
  - 普通业务流程里的“当前对象不存在” -> `BizException`
  - 明确的资源访问语义缺失 -> `ResourceNotFindException`
- 不要把“对象不存在”默认写成 `IllegalArgumentException("xxx 不存在")` 或 `IllegalStateException("xxx 不存在")`。

### 4.4 HTTP 状态异常

- 只有在接口确实需要明确区分 HTTP 状态时，才使用 `BaseHttpRuntimeException` 或等价异常。
- 典型场景：
...
- `BaseHttpRuntimeException`
  - 用于明确 HTTP 状态语义，典型是 401/403/协议协商类错误。
- `ResourceNotFindException`
  - 用于明确 REST 资源缺失的 404 语义，不用于所有“业务对象不存在”。
- 领域专用异常
  - 当某一类异常在模块内会反复出现，且需要稳定表达固定语义时，可以保留专门异常类。
  - 例如：
    - `DbAccessException`
...
- 优先基于 Athena 现有体系扩展，不要直接从 `RuntimeException` 裸继承。
- 如果没有特殊 HTTP 或协议语义，优先不要新建异常类，直接使用 `BizException`。
- 需要 HTTP 状态语义时，优先基于 `BaseHttpRuntimeException`。
- 自定义异常至少要满足以下之一：
  - 固定业务码
  - 固定 HTTP 状态
  - 固定领域语义
...
- 如果某个接口为了兼容协议必须返回特殊 HTTP 状态，再在 controller 或 facade 层抛 `BaseHttpRuntimeException`。
